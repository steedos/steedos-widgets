...
const filterToConditionItem = (filter) => {
  if (filter.length === 3) {
    const op = lodash.findKey(opMaps, (value) => {
      return value === filter[1];
    });
    if (
      op === "between" &&
      lodash.includes(DATE_DATETIME_BETWEEN_VALUES, filter[2])
    ) {
      return {
        left: {
          type: "field",
...
export const conditionsToFilters = (conditions) => {
  return conditionGroupToFilters(conditions);
};

export const filtersToConditions = (filters) => {
