...
// ============================================================
// 测试组 8: Bug 7 — 数学分组方括号 [] 应转换为圆括号 ()
// ============================================================
describe('mapFormula - Bug 7: 数学分组方括号转换', () => {

  test('方括号分组: [{a}+{b}]/{c}*100 → ${(a+b)/c*100}', () => {
    expect(mapFormula('[{a}+{b}]/{c}*100', null)).toBe('${(a+b)/c*100}');
  });

  test('实际百公里油耗公式: [{油量（本地）} + {油量（异地）}]/{本月里程数}*100', () => {
    expect(mapFormula('[{油量（本地）} + {油量（异地）}]/{本月里程数}*100', null))
      .toBe('${(油量_本地 + 油量_异地)/本月里程数*100}');
  });

  test('嵌套方括号: [[{a}+{b}]*{c}]/{d}', () => {
    expect(mapFormula('[[{a}+{b}]*{c}]/{d}', null)).toBe('${((a+b)*c)/d}');
  });

  test('无方括号的公式不受影响: {a} + {b}', () => {
    expect(mapFormula('{a} + {b}', null)).toBe('${a + b}');
  });
});

// ============================================================
// 测试组 9: 实际业务表单公式验证（单车核算明细表）
// ============================================================
describe('mapFormula - 实际业务公式: 单车核算明细表', () => {

  test('本月里程数: {月末里程数} - {月初里程数}', () => {
    expect(mapFormula('{月末里程数} - {月初里程数}', null))
      .toBe('${月末里程数 - 月初里程数}');
  });

  test('实际百公里油耗: [{油量（本地）} + {油量（异地）}]/{本月里程数}*100', () => {
    expect(mapFormula('[{油量（本地）} + {油量（异地）}]/{本月里程数}*100', null))
      .toBe('${(油量_本地 + 油量_异地)/本月里程数*100}');
  });

  test('本月发生额: 多字段求和', () => {
    const formula = '{燃油费（本地）} + {燃油费（异地）} + {维修保养费} + {过道费} + {停车费} + {洗车费} + {照证审验费} + {检车费} + {交强险} + {商业险} + {车船使用税} + {司机补助费} + {司机住宿费}';
    expect(mapFormula(formula, null))
