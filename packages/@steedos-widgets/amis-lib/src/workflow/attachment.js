...
window.previewAttachment = function(file) {
    ...
    // 构建完整 URL
    let fileUrl = window.location.origin + `/api/v6/files/cfs.instances.filerecord/${file._id}/${fileName}`;
    let downloadUrl = window.location.origin + `/api/v6/files/download/cfs.instances.filerecord/${file._id}/${fileName}`;
    ...
    if (['jpg', 'jpeg', 'png', 'gif', 'svg', 'bmp'].includes(fileExt)) {
        ...
        previewContent = React.createElement(AmisOfficeViewer, { src: fileUrl });
            ...
            previewContent = React.createElement('div', { className: "flex flex-col items-center justify-center h-full text-gray-500" }, [
                ...
                React.createElement('p', null, "此文件格式不支持在线预览"),
