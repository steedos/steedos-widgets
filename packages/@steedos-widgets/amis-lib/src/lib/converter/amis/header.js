...
export function getObjectListHeaderFirstLine(objectSchema, listViewName, ctx) {
  ...
  each(
    ...
    (listView, name) => {
      ...
      const getDialogAction = (listView, isCustom = false) => {
        ...
        if (isCustom) {
          // 先经过自定义的列表视图，关闭弹框时都要刷新
          dialogAction.dialog.onEvent = {
            "cancel": {
              "weight": 0,
              "actions": [
                {
          ...
          dialogAction.expression = `${!!listView.owner}`
        }
        return dialogAction;
      };
        ...
        body: [
          {
            ...
            "items": [
              {
                "type": "tpl",
                "tpl": listView.label + `${!listView.owner?'(元数据)':''}`
              },
              {
                "type": "button",
                "className": "steedos-listview-edit-button",
                "icon": "fa fa-edit",
                "hiddenOn": `!(global.user.is_space_admin || global.userId == '${listView.owner || ""}')`,
                "onEvent": {
                  "click": {
                    "actions": [
                      {
                        "actionType": "custom",
                        "script": `
                          const object_name = 'object_listviews';
                          const record_id = '${listView._id}';
                          ...
                          Creator.odata.insert(object_name, newRecord, function(result, error){
                              if(result){
                                ...
                                const dialogAction = ${JSON.stringify(getDialogAction(listView,true))};
                                dialogAction.dialog.data.recordId = result._id;
                                dialogAction.dialog.data._id = result._id;
                                doAction(dialogAction);
                              }
                          });
                        `,
                        "expression": `${!listView.owner} && global.user.is_space_admin`
                      },
                      getDialogAction(listView)
                    ]
                  }
                }
              }
