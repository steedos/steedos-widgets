                              ...
                              if (groupName === 'undefined' || groupName === '') {
                                  _.each(tabs, (tab) => {
                                      ...
                                      data.nav.push({
                                          ...
                                          "target":tab.target,
                                          "id": tab.id,
                                          "activeOn": "${pathname == '"+ tab.path +"' || STARTSWITH(pathname, '"+ tab.path +"/')}",
                                          "index": tab.index,
                                                    ...
                                                    "actions": [
                                                        {
                                                            ...
                                                            "api": {
                                                                "url": "/graphql",
                                                                "method": "post",
                                                                "adaptor": "var locale = Builder.settings.context?.user?.language || window.navigator.language; const tabs = payload.data.tabs; if(tabs && tabs.length > 0){ try{const pageId = tabs[0].page__expand._id; window.location.href=Steedos.getRelativeUrl('/api/pageDesign?pageId=' + pageId +\`&assetUrls=\${Builder.settings.assetUrls}\`+'&retUrl='+window.location.href+'&locale='+locale)}catch(e){payload.error=e.message;} }; return payload;",
