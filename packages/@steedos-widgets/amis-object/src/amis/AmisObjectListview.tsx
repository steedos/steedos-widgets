 ...
 * @LastEditTime: 2025-10-14 16:05:27
 * @Description: 
 */
...
export const AmisObjectListView = async (props) => {
      ...
      "actions": [
        {
          ...
          "actionType": "link",
          "expression": "${!!!event.data.recordId && event.data.__deletedRecord != true && event.data._isRelated != true && event.data.reload !=true && !event.data.__saveAndNewRecord}" //是新建, 则进入详细页面. 
        },
        {
          "actionType": "reload",
  ...
  const schema = {
    ...
    data: _data,
    style: style,
    className: `${className} h-full  steedos-object-listview ${displayAs === 'split'? 'w-[24rem] xl:w-[24rem] 2xl:w-[32rem] ':''}`,
    body: [{
      "type": "wrapper",
      "size": "none",
